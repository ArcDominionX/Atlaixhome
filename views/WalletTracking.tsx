    ...
    useEffect(() => {
        if (viewMode === 'profile' && netWorthChartRef.current && typeof ApexCharts !== 'undefined') {
            const options = {
                ...
                yaxis: {
                    labels: {
                        style: { colors: '#8F96A3', fontFamily: 'Inter' },
        ...
        return (
            <div className="flex flex-col gap-6 h-full">
                ...
                <div className="flex flex-col xl:flex-row gap-6 items-start h-full">
                    ...
                    <div className="flex-1 w-full min-w-0">
                        ...
                        <div className="flex flex-wrap gap-3 items-center mb-6">
                            <div className="filter-wrapper relative">
                                <div className={`filter-pill ${activeFilter === 'chain' ? 'active' : ''}`} onClick={() => toggleFilter('chain')}>
                            ...
                            <div className="filter-wrapper relative">
                                <div className={`filter-pill ${activeFilter === 'type' ? 'active' : ''}`} onClick={() => toggleFilter('type')}>
                            ...
                            <div className="flex-1 min-w-[200px] bg-card border border-border rounded-lg flex items-center px-4 py-2">
                                <Search size={16} className="text-text-medium mr-2" />
                                <input type="text" className="bg-transparent border-none text-text-light outline-none w-full text-sm" placeholder="Search wallet..." />
                            </div>
                        </div>
                                ...
                                <div key={w.id} className="bg-card border border-border rounded-xl p-5 hover:border-text-dark transition-colors flex flex-col">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 bg-[#2A2E33] rounded-full"></div>
                                    ...
                                    <div className="mb-4">
                                        <div className="text-xs text-text-medium mb-1">Total Portfolio Value</div>
                                        <div className="text-2xl font-bold">{w.bal}</div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4 mb-6">
                                        <div><div className="text-xs text-text-dark mb-1">30d PnL %</div><div className={`font-semibold text-sm ${w.pnl.includes('+') ? 'text-primary-green' : ''}`}>{w.pnl}</div></div>
                                        <div><div className="text-xs text-text-dark mb-1">Win Rate %</div><div className="font-semibold text-sm">{w.win}</div></div>
                                        <div><div className="text-xs text-text-dark mb-1">% Tokens Held</div><div className="font-semibold text-sm">45% Top 3</div></div>
                                        <div><div className="text-xs text-text-dark mb-1">Last Activity</div><div className="font-normal text-sm">{w.time}</div></div>
                                    </div>
                                    <div className="flex gap-3 mt-auto">
                                        <button className="flex-1 py-2 border border-border rounded-lg text-xs font-semibold hover:bg-card-hover hover:text-text-light transition-all" onClick={() => openWallet(w)}>View Wallet</button>
                                        <button className="flex-1 py-2 border border-border rounded-lg text-xs font-semibold hover:bg-card-hover hover:text-text-light transition-all">Set Alerts</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        {visibleCount < wallets.length && (
                            <div className="w-full mt-6 py-3 border border-dashed border-border text-text-medium text-center rounded-xl cursor-pointer hover:bg-card-hover hover:text-text-light transition-all" onClick={() => setVisibleCount(prev => prev + 4)}>
                                See More
                            </div>
                        )}
                    </div>
                    ...
                    <div className="w-full xl:w-[340px] flex-shrink-0 bg-card border border-border rounded-xl p-5 sticky top-6">
                        <div className="mb-4 pb-4 border-b border-border">
                            <h3 className="text-lg font-semibold text-text-light">Global Wallet Activity</h3>
                        </div>
                        <div className="flex gap-2 overflow-x-auto pb-2 mb-2 scrollbar-hide">
                            {['By Wallet', 'By Token', 'By Chain', 'By Action'].map(f => (
                                <div key={f} className={`flex-shrink-0 text-xs px-2.5 py-1 rounded border cursor-pointer transition-colors ${f === 'By Wallet' ? 'bg-card-hover border-text-light text-text-light' : 'bg-main border-border text-text-medium'}`}>{f}</div>
                            ))}
                        </div>
                        <div className="flex flex-col gap-3 max-h-[700px] overflow-y-auto pr-1">
                            {globalActivity.map((item, idx) => (
                                <div key={idx} className="bg-gradient-to-br from-[#222529] to-[#1C1F22] rounded-lg p-3 border border-border hover:border-text-medium transition-colors">
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="font-mono text-xs text-primary-purple font-semibold flex items-center gap-1.5">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <img src={item.coinIcon} className="w-4.5 h-4.5 rounded-full" />
                                        <div className="font-bold text-sm text-text-light">{item.amount}</div>
                                    </div>
                                    <div className="flex justify-between items-center mt-2 text-xs text-text-dark">
                                        <div className="flex items-center gap-1"><ChevronDown size={10} /> {item.time}</div>
                                        <div className="cursor-pointer border border-border px-2 py-0.5 rounded hover:text-text-light hover:border-text-light transition-colors">View</div>
                                    </div>
                                </div>
                            ))}
                        </div>
    ...
    if (viewMode === 'profile') {
        return (
            <div className="flex flex-col gap-6">
                <div 
                    className="flex items-center gap-2 text-text-medium hover:text-text-light cursor-pointer mb-2 w-fit transition-colors"
                ...
                <div className="bg-main border border-border rounded-xl p-6 flex flex-col md:flex-row items-start md:items-center gap-4 justify-between">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <div className="flex-1 md:w-[300px] bg-[#0A0B0D] border border-border rounded-lg flex items-center px-4 py-2">
                        ...
                        <button className="bg-primary-green text-main px-4 py-2 rounded-lg font-semibold hover:bg-primary-green-darker transition-colors whitespace-nowrap">+ Track</button>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <span className={`text-sm px-3 py-1 rounded font-bold uppercase ${
                    ...
                    <div className="flex flex-wrap gap-x-8 gap-y-4 items-center">
                        <div>
                            <div className="text-xs text-text-medium">Total Value</div>
                            <div className="text-xl font-bold">{selectedWallet?.bal}</div>
                        </div>
                        <div>
                            <div className="text-xs text-text-medium">30d PnL</div>
                            <div className="text-xl font-bold text-primary-green">{selectedWallet?.pnl}</div>
                        </div>
                        <div>
                            <div className="text-xs text-text-medium">Win Rate</div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-card border border-border rounded-xl p-5 text-center">
                        <h5 className="text-text-medium text-xs mb-1">Total Holdings</h5>
                        <p className="text-text-light font-bold text-lg">{selectedWallet?.bal}</p>
                    </div>
                    <div className="bg-card border border-border rounded-xl p-5 text-center">
                        <h5 className="text-text-medium text-xs mb-1">Active Positions</h5>
                        <p className="text-text-light font-bold text-lg">8</p>
                    </div>
                    <div className="bg-card border border-border rounded-xl p-5 text-center">
                        <h5 className="text-text-medium text-xs mb-1">Profitable Trades</h5>
                        <p className="text-primary-green font-bold text-lg">142</p>
                    </div>
                    <div className="bg-card border border-border rounded-xl p-5 text-center">
                        <h5 className="text-text-medium text-xs mb-1">Avg Hold Time</h5>
                ...
                <div className="bg-card border border-border rounded-xl p-6">
                    <h3 className="card-title">Net Worth Performance</h3>
                    <div ref={netWorthChartRef} className="w-full min-h-[250px]"></div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
                        <h3 className="card-title">Portfolio Breakdown</h3>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                <thead>
                                    <tr className="border-b border-border text-text-dark text-xs uppercase">
                                ...
                                <tbody>
                                    {portfolio.map((p, i) => (
                                        <tr key={i} className="border-b border-border last:border-0 hover:bg-card-hover/50 transition-colors">
                                            ...
                                            <td className="py-3 text-text-medium">{p.amount}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
                        ...
                        <div className="flex flex-col">
                            <div className="flex gap-4 py-4 border-b border-border">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                        <span className="font-semibold text-primary-green">Bought $PEPE</span>
                                        <span className="text-xs text-text-dark">2m ago</span>
                                    </div>
                                    <div className="text-sm text-text-medium">Swapped 2.5 ETH for 4.2B PEPE</div>
                                </div>
                            </div>
                            <div className="flex gap-4 py-4 border-b border-border">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                        <span className="font-semibold text-primary-red">Sold $WIF</span>
                                        <span className="text-xs text-text-dark">15m ago</span>
                                    </div>
                                    <div className="text-sm text-text-medium">Sold 50% position for $12k Profit</div>
                                </div>
                            </div>
                            <div className="flex gap-4 py-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between mb-1">
                                        <span className="font-semibold text-primary-blue">Added Liquidity</span>
                                        <span className="text-xs text-text-dark">2h ago</span>
                                    </div>
                                    <div className="text-sm text-text-medium">Added 10 ETH to UNI-V2 Pool</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {recentTx.map((tx, i) => (
                                        <tr key={i} className="border-b border-border last:border-0 hover:bg-card-hover/50 transition-colors">
                                            <td className="py-3 font-mono text-primary-blue cursor-pointer hover:underline">{tx.hash}</td>
                                            <td className="py-3"><span className="px-2 py-0.5 bg-[#2A2E33] rounded text-xs">{tx.method}</span></td>
                                            <td className="py-3 text-text-medium">{tx.age}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
